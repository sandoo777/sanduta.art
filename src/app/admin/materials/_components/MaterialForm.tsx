    ...
    <form onSubmit={submitForm} className="p-6 space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <FormLabel>Preț achiziție (MDL)</FormLabel>
            <Input {...form.register('purchasePrice')} type="number" step="0.01" />
          ...
          <div>
            <FormLabel>
              Preț vânzare (MDL) {consumptionType === 'DIRECT' ? '*' : ''}
            </FormLabel>
          ...
          {form.watch('salePriceMode') === 'percent' && (
            <div>
              <FormLabel>Adaos (%)</FormLabel>
              <Input {...form.register('salePricePercent')} type="number" step="0.1" min="0" />
          ...
          {selectedCategory && (
            <div className="md:col-span-2 text-xs text-gray-500 bg-gray-50 border border-gray-200 rounded-md p-3">
              Unități recomandate pentru această categorie:
              {' '}
              {[
                selectedCategory.requiresPricePerSqm ? 'm2' : null,
                selectedCategory.requiresPricePerMeter ? 'meter' : null,
                selectedCategory.requiresPricePerUnit ? 'unit/pcs' : null,
              ].filter(Boolean).join(', ') || 'fără restricții explicite'}
            </div>
          )}
